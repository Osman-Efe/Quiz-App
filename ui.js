...
UI.prototype.showQuestion = function(soru){
    ...
    title.textContent = soru.que;

    const optionList = document.createElement("div");
    optionList.classList.add("option-list");

    for(let [key,value] of Object.entries(soru.ans)){
...
UI.prototype.showNumberofQuestion = function(queryofQuestion, sumQuestion){
    const tag = `<span class=" badge text-bg-danger">${queryofQuestion} / ${sumQuestion}</span>`;
    document.querySelector(".question-index").innerHTML = tag;
}

UI.prototype.showScore = function(trueQuestionNumber , sumQuestion){
    const tag = `Toplam ${sumQuestion} soruda ${trueQuestionNumber} doğru cevap verdiniz.`;
